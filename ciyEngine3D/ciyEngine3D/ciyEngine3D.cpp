...
#include <strstream>    //File handling functions...
#include <algorithm>
#include <string>
        ...
        while (!f.eof()) {
            ...
            if (!bHasTexture) {
                if (line[0] == 'f') {
                    int f[3];
                    s >> junk >> f[0] >> f[1] >> f[2];
                    tris.push_back({ verts[f[0] - 1], verts[f[1] - 1], verts[f[2] - 1] });  //since counting from one not zero
                }
            }
            else {
                if (line[0] == 'f')
...
private :
    ...
    vec3d vCamera;  //Location of camera in world Space (EYE)
    vec3d vLookDir; //Direction vector along the direction camera points (EYE - AT)
    ...
    vec3d Matrix_MultiplyVector(mat4 &m, vec3d &i) {
        vec3d v;
    ...
    mat4 Matrix_PointAt(vec3d& eye, vec3d& target, vec3d& up) {
        vec3d newForward = Vector_Sub(target, eye);
...
public :
    bool OnUserCreate() override {
        ...
        pDepthBuffer = new float[ScreenWidth() * ScreenHeight()];   //every pixel on screen
    ...
    bool OnUserUpdate(float fElapsedTime) override {
        ...
        Fill(0, 0, ScreenWidth(), ScreenHeight(), PIXEL_SOLID, FG_BLACK);           //initialize screen
        ...
        vec3d vUp = { 0, 1, 0 };
        vec3d vTarget = { 0, 0, 1 };
        mat4 matCameraRot = Matrix_MakeRotationY(fYaw);
        vLookDir = Matrix_MultiplyVector(matCameraRot, vTarget);
        vTarget = Vector_Add(vCamera, vLookDir);

        mat4 matCamera = Matrix_PointAt(vCamera, vTarget, vUp);

        mat4 matView = Matrix_QuickInverse(matCamera);
        ...
        olcSprite* tex)
    {
        ...
        if (dy1) {
            for (int i = y2; i <= y3; i++) {
                ...
                float tex_eu = u1 + (float)(i - y1) * du2_step;
                float tex_ev = v1 + (float)(i - y1) * dv2_step;
                float tex_ew = v1 + (float)(i - y1) * dw2_step;
                ...
                for (int j = ax; j < bx; j++) {
                    ...
                    if (tex_w > pDepthBuffer[i * ScreenWidth() + j]) {  //Depth Test
